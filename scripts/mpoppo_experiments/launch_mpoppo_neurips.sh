...
###############################################################################
if (( $# < 3 )); then
  echo "Usage: $0 <pem_key> <rm_address> <mrm_address>"
  exit 1
fi
...
run_experiment() {
    ...
    #  GPU layout
    # ------------------------------------------------------------------------
    local CUDA_DEVICES="0,1,2"
    local ACC_CONFIG="$trl_dir/examples/accelerate_configs/deepspeed_zero2_3gpus.yaml"
    ...
    accelerate launch --config_file "$ACC_CONFIG" \
        ...
        --local_rollout_forward_batch_size 36 \
