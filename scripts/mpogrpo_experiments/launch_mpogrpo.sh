...
rm_address=$2
mrm_address=$3
remote_host=$4   # default unchanged
...
TASK="math_reasoning"
PROMPT_DIR="$trl_dir/trl/extras/mpogrpo/prompts/math_reasoning"
...
run_experiment() {
    ...
    [[ "$exp_type" == "mpogrpo" ]] && num_mpo_interval=10
    ...
    # ------------------------------------------------------------------------
    # bash "$trl_dir/scripts/mpogrpo_experiments/launch_rm_mrm.sh" \
    #     "$pem_key" "$remote_host" \
    #     "${rm^^}" "${mrm^^}"
