...
def prepare_essay_writing_dataset(tokenizer, split: str = "test", train_size: int = None):
    ...
    def _prepare_dataset(dataset, tokenizer):
        def tokenize(sample):
            prompt_text = (
                "You will act as an English writer and compose either an essay or a story depending on the instruction given below. "
                "Your essay should be no more than 350 words.\n\nInstructions:\n"
                + sample["prompt"]
                + "\n\nYour Writing:\n"
            )
            ...
            return {"input_ids": input_ids, "lengths": len(input_ids), "prompt": prompt_text}
...
def prepare_mathematical_reasoning_dataset(tokenizer, split: str, data_file_path: list[str]):
    ...
    def tokenize(sample):
        ...
        if answer is None:
            print(sample)
        prompt_text = (
            'You will take on the role of a mathematician, solving a given math problem with clear mathematical reasoning, and present the final answer enclosed in "\\boxed{ }". '
            "Your solution and answer should be in Latex equation format. For example, followed by a solution, an answer can be presented as : "
            '"\\boxed{3 \\text{cm}}" or "\\boxed{\\frac{2}{3}}".\n\nProblem:\n'
            + sample["problem"]
...
def prepare_summarization_dataset(tokenizer, split: str = "test", train_size: int = None):
    def tokenize(sample):
        prompt_text = (
            "Acting as an experienced government official, write a concise and faithful summary of the following US Congressional and California state bill. "
...
def prepare_ethical_reasoning_dataset(tokenizer, split: str, data_file_path: str, train_size: int = None):
    ...
    def tokenize(sample):
        ...
        return {"input_ids": input_ids, "lengths": len(input_ids), "prompt": prompt_text, "verdict": verdict}
