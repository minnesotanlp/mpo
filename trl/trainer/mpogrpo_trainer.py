...
from .mpogrpo_config import MPOGRPOConfig
from .utils import (
...
class MPOGRPOTrainer(Trainer):
    """
    MPOGRPO: GRPO core (group-relative, ref/value-free) with MPO meta-reward/prompt updates.
    """

    _tag_names = ["trl", "mpogrpo"]

    def __init__(
        self,
        model: Union[str, PreTrainedModel],
        args: Optional[MPOGRPOConfig] = None,
        ...
        if args is None:
            ...
            args = MPOGRPOConfig(f"{model_name}-MPOGRPO")
        ...
        if isinstance(model, str):
            ...
            if isinstance(torch_dtype, torch.dtype) or torch_dtype == "auto" or torch_dtype is None:
                ...
                raise ValueError(
                    "Invalid `torch_dtype` passed to `MPOGRPOConfig`. Expected either 'auto' or a string representing "
            ...
            if args.model_init_kwargs is not None:
                raise ValueError(
                    "You passed `model_init_kwargs` to the `MPOGRPOConfig`, but your model is already instantiated. "
        ...
        # Disable dropout in the model (no ref model for MPOGRPO)
        ...
        # MPO reward/meta-reward models
        self.experiment_directory = args.output_dir
        ...
        source_prompts_directory = os.path.join(os.path.dirname(mpogrpo.__file__), "prompts", args.task_name)
        os.makedirs(self.prompts_directory, exist_ok=True)
        # seed prompts
        if args.task_name == "math_reasoning":
            ...
            for domain in math_domains_to_idx.keys():
                for cluster_idx in range(1, self.cluster_size + 1):
                    ...
                    if args.init_rm_prompt is not None:
        ...
        self.rm_kwargs = {
            ...
            "reward_model_address": args.reward_model_address,
            "experiment_directory": self.experiment_directory,
        }
        self.mrm_kwargs = {
            "task_name": args.task_name,
            "reward_model_address": args.meta_reward_model_address,
            "experiment_directory": self.experiment_directory,
        }
        if args.task_name == "math_reasoning":
            self.rm_kwargs["cluster_size"] = 3
            self.mrm_kwargs["cluster_size"] = 3
        self.reward_model = mpogrpo.get_reward_model(**self.rm_kwargs)
        self.meta_reward_model = mpogrpo.get_meta_reward_model(**self.mrm_kwargs)
        self.reward_func_names = ["mpogrpo_rm"]
        self.reward_processing_classes = []
        self.reward_funcs = []
        self._mpo_rollouts: list[dict[str, Any]] = []
        ...
        # Compatibility patch: some accelerate versions don't accept keep_torch_compile kwarg
        if hasattr(self, "accelerator") and callable(getattr(self.accelerator, "unwrap_model", None)):
            orig_unwrap = self.accelerator.unwrap_model

            def _compat_unwrap(model, *a, **kw):
                return orig_unwrap(model)

            self.accelerator.unwrap_model = _compat_unwrap
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: MPOGRPOConfig) -> PreTrainedModel:
    ...
    def _generate_and_score_completions(
        ...
        if "prompt" in inputs[0]:
            ...
            # fallback to provided input_ids/attention_mask
            ids = [torch.tensor(x["input_ids"], device=device) for x in inputs]
            prompt_ids = pad(ids, padding_value=self.processing_class.pad_token_id)
            masks = [torch.tensor([1] * len(x["input_ids"]), device=device) for x in inputs]
            prompt_mask = pad(masks, padding_value=0, padding_side="left")
        ...
        # MPO reward scoring (single RM)
        with profiling_context(self, "mpogrpo_rm"):
            # repeat any extra fields to align with completions if needed
            ...
            rewards = torch.tensor(score, dtype=torch.float32, device=device)
            # store evaluations for potential meta updates
            self._latest_mpo_evaluations = evaluation
        ...
        mean_rewards = torch.nanmean(rewards).item()
        self._metrics[mode][f"rewards/{self.reward_func_names[0]}/mean"].append(mean_rewards)
        std_rewards = nanstd(rewards).item()
        self._metrics[mode][f"rewards/{self.reward_func_names[0]}/std"].append(std_rewards)
        ...
        gathered_rewards = rewards.tolist()
        gathered_evals = gather_object(getattr(self, "_latest_mpo_evaluations", [{} for _ in gathered_rewards]))
        self._textual_logs["prompt"].extend(gathered_prompts)
        self._textual_logs["completion"].extend(gathered_completions)
        self._textual_logs["rewards"][self.reward_func_names[0]].extend(gathered_rewards)

        # Accumulate rollouts for meta updates (only main process keeps the buffer)
        if self.accelerator.is_main_process:
            # capture any extra task-specific fields present in the batch
            extra_keys = [k for k in inputs[0].keys() if k not in ["prompt", "completion"]]
            extras = {k: gather_object([inp[k] for inp in inputs]) for k in extra_keys}
            for idx, (p, c, r) in enumerate(zip(gathered_prompts, gathered_completions, gathered_rewards)):
                rollout_entry = {
                    "queries": p,
                    "student_responses": c,
                    "junior_scores": to_serializable(r),
                    "junior_evaluations": gathered_evals[idx] if idx < len(gathered_evals) else {},
                }
                for k, v_list in extras.items():
                    rollout_entry[k] = to_serializable(v_list[idx])
                self._mpo_rollouts.append(rollout_entry)

        # MPO interval handling: serialize rollouts/evaluations and refresh RMs periodically
        if mode == "train" and self.state.global_step > 0 and self.state.global_step % self.args.num_mpo_interval == 0:
            self.accelerator.wait_for_everyone()
            if self.accelerator.is_main_process:
                # Write rollouts snapshot
                rollouts_path = os.path.join(
                    self.rollouts_directory, f"rollouts-{self.state.global_step}-{self.accelerator.device}.json"
                )
                with open(rollouts_path, "w") as f:
                    f.write(json.dumps(self._mpo_rollouts, indent=2))
                # Meta-evaluate and update prompts
                evaluation_logs = self.meta_reward_model.meta_evaluate_and_update(
                    batch_index=self.state.global_step,
                    return_evaluations=True,
                    num_samples=self.args.num_mpo_samples,
                    do_prescreening=False,
                )
                eval_path = os.path.join(
                    self.evaluations_directory, f"meta_evaluation_results_{self.state.global_step}.json"
                )
                with open(eval_path, "w") as f:
                    f.write(json.dumps(evaluation_logs, indent=2))
                # Reset buffer after checkpoint
                self._mpo_rollouts = []
            self.accelerator.wait_for_everyone()
            # reload RM/MRM to pick up prompt updates
            self.reward_model = mpogrpo.get_reward_model(**self.rm_kwargs)
            self.meta_reward_model = mpogrpo.get_meta_reward_model(**self.mrm_kwargs)
