...
- **Additional scripts** for launching remote LLM servers and evaluating trained models are provided in `scripts/mpoppo_experiments`.
...
- The `launch_mpoppo.sh` script in `scripts/mpoppo_experiments` demonstrates how to train models using MPOPPO with different parameter configurations.
- The `launch_rm_mrm.sh` script in `scripts/mpoppo_experiments` shows how to instantiate and serve LLMs via SGLang over an SSH connection.
